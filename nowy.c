...
void get_pixel_color (Display *d, int x, int y, XColor *color) {
  XImage *image;
  image = XGetImage (d, XRootWindow (d, DefaultScreen (d)), x, y, 1, 1, AllPlanes, XYPixmap);
   ...
   while (1)
   {
      ...
      switch (myevent.type)
      {
		  case ButtonPress:
              ...
              /*Window root = XDefaultRootWindow(mydisplay);
              XImage *image;
              image = XGetImage(mydisplay, XDefaultRootWindow(mydisplay), xw1, yw1, 1, 1, AllPlanes, XYPixmap);
              printf("pixel: %lu\n", XGetPixel(image, 0, 0)); */
              XColor c;
			  get_pixel_color (mydisplay, xw1, yw1, &c);
